...
function App() {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL.lecture}/lecturelist`).then((response) => {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL.lecture}/lecturelist/recommend`).then((response) => {
  ...
  useEffect(() => {
    axios
      .get(`${BASE_URL.group}/info`)
      // axios.get(`${BASE_URL.react}/assets/group_data.json`)
      .then((response) => {
        // setGroupData(response.data._embedded.info)
        setGroupData(response.data._embedded.info).catch((err) =>
          console.log(err)
        );
      });
  }, []);
  ...
  useEffect(() => {
    axios.get(`${BASE_URL.react}/assets/group_data.json`).then((response) => {
          ...
          <GroupStateContext.Provider value={groupData}>
            ...
            <BrowserRouter>
              <div className="App">
                ...
                <MyHeader
                  head_Home={"Home"}
                  head_lecture={"lecture"}
                  head_group={"Group"}
                  head_chatting={"Chatting"}
                  head_mypage={"MyPage"}
                  head_login={"LogIn"}
                  head_signup={"SignUp"}
                />
                ...
                <Routes>
                  ...
                  <Route
                    path="/group/gallery/new/:groupId"
                    element={<GalleryNew />}
                  />
                  <Route
                    path="/group/post/new/:groupId"
                    element={<PostItemNew />}
                  />
                  <Route
                    path="/group/post/edit/:groupId/:postId"
                    element={<PostItemEdit />}
                  />
                  <Route path="/group/post/:id" element={<Post />} />
                  <Route
                    path="/group/post/:groupId/:postId"
                    element={<PostItem />}
                  />
